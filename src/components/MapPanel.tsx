...
const SEOUL_BOUNDS = L.latLngBounds([37.42, 126.76], [37.70, 127.18])
...
function MapContent({
  ...
  return (
    <>
      <KoreaGeoJSONLayer sidoCounts={sidoCounts} onRegionSelect={onRegionSelect} />
...
export function MapPanel({ region, onRegionChange, treeData, seoulTreeCount }: MapPanelProps) {
  ...
  useEffect(() => {
    ...
    if (region === "11") {
      map.fitBounds(SEOUL_BOUNDS, { padding: [40, 40], maxZoom: 12 })
      prevRegionRef.current = "11"
    } else if (region === "41") {
      map.fitBounds(GYEONGGI_BOUNDS, { padding: [40, 40], maxZoom: 10 })
      prevRegionRef.current = "41"
    } else if ((prevRegionRef.current === "11" || prevRegionRef.current === "41") && region === "00") {
  ...
  return (
    <main className="flex-1 flex flex-col min-w-0 bg-gray-100 relative">
      <div className="absolute left-4 bottom-16 z-[1000] bg-white rounded-md shadow-md p-1.5 text-xs">
        <div className="font-semibold text-gray-800 mb-0.5 text-[11px]">
          색깔 설명 (단위: 그루){region === "41" ? " · 경기" : ""}
        </div>
        <div className="space-y-0.5">
          <div className="flex items-center gap-1">
            <div
              className="w-2.5 h-2.5 rounded-sm border border-gray-300 shrink-0"
          ...
          {getFourStepLabels(nationalFourStep.breaks).map((label, i) => ({
            color: nationalFourStep.colors[i],
            label,
          })).map(({ color, label }) => (
          ...
          {region === "11" && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-[1000]">
              <a
                ...
                className="pointer-events-auto group flex flex-col items-center gap-3 rounded-2xl border border-white/20 bg-white/95 px-10 py-6 shadow-[0_8px_32px_rgba(0,0,0,0.12)] backdrop-blur-sm transition-all duration-300 hover:shadow-[0_12px_40px_rgba(0,0,0,0.16)] hover:scale-[1.02]"
              >
      ...
      <div className="h-9 shrink-0 flex items-center justify-center gap-2 bg-slate-50/95 border-t border-slate-200 text-[11px] text-slate-500 tracking-wide">
